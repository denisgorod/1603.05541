...
####  by Anders Bj�rner, KTH Stockholm, Sweden,               ####
...
type_of_object:=1; # <--- 1 = test object; write the facets  
                   #          in the format
                   #          
                   ...
                   ###################################################
                                                                     #
                              file:=String("BISTELLAR.testobject");  #
                                                                     #
                   ###################################################
                   #
                   #      2 = boundary of the cyclic polytope
                   #          of  polytope_dimension  with  number_of_vertices 
                   #      3 = boundary of the cross polytope
                   #          of  polytope_dimension
                   #      4 = suspended Poincar� homology 3-sphere
                   #          with  number_of_suspensions
                   #      5 = 9-vertex torus
                   #
                   #########
                           #
polytope_dimension:=8;     #
number_of_vertices:=105;   #
number_of_suspensions:=5;  #
...
##                                                              ##
## (d) k-fold suspensions of the Poincar� homology 3-sphere     ##
##     give examples of non-PL (k+3)-spheres for k >= 2.        ##
##     In particular, the 2-fold suspension of the provided     ##
##     16-vertex triangulation of the Poincar� sphere yields    ##
...
##     Poincar� homology 3-sphere yield non-PL d-spheres        ##
...
facets:=[]; dim:=0; max:=0; faces:=[]; maxvertex:=0;
f:=[]; h:=[]; g:=[];
chi:=0; sum:=0;
raw_options:=[];
options:=[];
...
##
Add(bisfaces,ShallowCopy(faces[max]));
##
...
##
	Add(randomelements, ['-', Length(randomelement[1])-1,randomelement]);
##
                elif verbose = 2 then
                   Print("\n","Reverse-",Length(randomelement[1])-1,"-Move   (",rounds," rounds)");
                   AppendTo(log_file,"\n","Reverse-",Length(randomelement[1])-1,"-Move   (",rounds," rounds)");
